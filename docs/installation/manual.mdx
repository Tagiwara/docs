...
Folge diesen Schritten, um die richtige Version von evcc herunterzuladen:
...
4. Es gibt nun einen neuen Ordner mit dem Programm `evcc` und einer Beispielkonfiguration `evcc.dist.yaml` welche bearbeitet werden muss.
...
- Prüfe, ob evcc korrekt installiert wurde:
  `which evcc`
  Die Ausgabe sollte so aussehen: `/usr/local/bin/evcc`
...
- Lass evcc nach einigen unterstützen Geräten im Netzwerk suchen und führe folgenden Befehl aus:
  `evcc detect`
  evcc wird beginnen, das lokale Netzwerk zu duchsuchen. Das kann etwas dauern.
...
- Falls `evcc detect` einen `ping: socket: permission denied` Fehler anzeigt, muss der folgende Befehl ausgeführt und anschließend der vorherige Schritt wiederholt werden:
  `sudo sysctl -w net.ipv4.ping_group_range="0 2147483647"`
- Kopiere nun die Beispiel-Konfigurationsdatei in das Verzeichnis `/etc` und benne die Datei um in `evcc.yaml`:
